...
def train_test_rep_split4(raw_data, channel, sep_criteria, is_test_only=False):
    ...
    for i in range(len(x)):
        if (rep[i] == 2 and sep_criteria == 'rep') \
                ...
                or (act[i] == int(sep_criteria[-2:]) and sep_criteria.startswith("mal")) \
                or (act[i] == int(sep_criteria[-2:]) and sep_criteria.startswith("fem")) \
                or (act[i] == int(sep_criteria[-2:]) and sep_criteria.startswith("sin")):
            _test_x.append(x[i])
            _test_y.append(y[i])
