...
sys.stdout = model_parameter.Logger(
    f'{root_path}/log/log_train_{mat_file_name.split(".")[0]}_{split_method}_{time.strftime("%Y%m%d-%H%M%S")}.log')
...
train_data, train_label, test_data, test_label, matrix_test_set = (
    dataset_split.train_test_rep_split5(digits, c, split_method))
...
model_save_path = root_path + 'model/model_' + mat_file_name.split('.')[0] + '_' + split_method + '.h5'
...
cnn_model = keras.models.load_model(model_save_path)
...
#     "NEU": 4,
#     "SAD": 5,
emotion_list = ["ANG", "DIS", "FEA", "HAP", "NEU", "SAD"]
for i in range(len(matrix_test_set)):
    test_set = np.array(matrix_test_set[i])
