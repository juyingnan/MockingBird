...
from bokeh.models import ColumnDataSource, CDSView, IndexFilter, TabPanel, Tabs
...
def create_sample_scatter(x_data, y_data, source, label, title='', x_axis_title='', y_axis_title='', unmute=''):
    ...
    for cat_filter in label['standard_label_list']:
        ...
        current = result_plot.scatter(x_data, y_data, source=source, fill_alpha=0.4, size=8,
                                      ...
                                      color=factor_cmap('label', Category20[20][:12], label['standard_label_list']),
...
markers = [
    'hex', 'triangle', 'circle', 'cross',
    'diamond', 'square', 'x', 'inverted_triangle',
    'asterisk', 'circle_x', 'triangle_dot', 'dash'
]
sample_labels = {
    'emotion':
        {
...
for mapping in ['tsne', 'umap']:
    output_file(root_path + f"vis/{mapping}_{feature_name}.html")
    ...
    for label_key, labels in zip(['statement', 'gender', 'emotion'], [statement_label, gender_label, emotion_label]):
        ...
        tab_list.append(TabPanel(child=column(plot_all, grid), title=label_key))
    ...
    for emo in emotions:
        ...
        grid = gridplot(grid_list, ncols=cols, width=400, height=350)
        tab_list.append(TabPanel(child=column(plot_all, grid), title=f'emotion-{emo}'))
